...
// 아두이노 Arduino RF24 코드의 address[6] = "00001" 과 동일
// '0'=0x30, '0'=0x30, '0'=0x30, '0'=0x30, '1'=0x31
static const uint8_t NRF_ADDR[5] = {0x30, 0x30, 0x30, 0x30, 0x31};
...
// ── 공용 설정 — 아두이노 RF24 설정과 일치시킴 ────────────────
// Arduino: setDataRate(RF24_250KBPS), setPALevel(RF24_PA_MIN)
//          auto-ACK 활성(기본), address "00001"
static void common_config(void) {
    write_reg(NRF_EN_AA,      0x01);   // Auto-ACK pipe 0 활성 (Arduino TX가 ACK 기대)
    write_reg(NRF_SETUP_AW,   0x03);   // 5바이트 주소
    write_reg(NRF_SETUP_RETR, 0x00);   // RX측 재전송 불필요
    write_reg(NRF_RF_CH,      NRF_CHANNEL);
    // RF_SETUP: bit5=RF_DR_LOW=1(250Kbps), bit3=RF_DR_HIGH=0,
    //           bit[2:1]=RF_PWR=00(-18dBm=PA_MIN), bit0=1
    write_reg(NRF_RF_SETUP,   0x21);
}
